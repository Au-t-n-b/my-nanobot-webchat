      ...
      <div className="rounded-xl border border-slate-100 overflow-hidden shadow-sm dark:border-white/5">
        <div className="overflow-x-auto">
          <table className="w-full border-collapse" style={{ minWidth }}>
            <thead>
              <tr className="border-b-2 border-slate-200 dark:border-[var(--border-strong)]">
                {safeColumns.map((col) => (
                  <th
                    key={col.key}
            ...
            <tbody>
              {safeRows.map((row, ri) => (
                <tr
                  key={ri}
                  className={[
                    "border-b border-slate-50 last:border-0 transition-colors dark:border-[var(--border-subtle)]",
                    canSelect ? "hover:bg-slate-50/50 dark:hover:bg-[var(--surface-2)]/30 cursor-pointer" : "",
                    selectedRowIndex === ri ? "bg-slate-50/70 dark:bg-[var(--surface-2)]/40" : "",
                  ]
                    .join(" ")
                    .trim()}
                  onClick={() => {
                ...
                >
                  {safeColumns.map((col) => {
                    ...
                    return (
                      <td key={col.key} className="px-4 py-4 text-sm leading-relaxed text-slate-700 align-top dark:text-[var(--text-secondary)]">
                        {canEdit ? (
                          <input
                            className="w-full bg-transparent border-b border-transparent focus:border-slate-400 focus:outline-none focus:ring-0 px-0 py-1 transition-colors dark:focus:border-[var(--border-strong)]"
                            value={display}
