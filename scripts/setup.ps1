...
# 避免双引号内 "[...]" 被 PowerShell 误解析为类型/索引；日志前缀用单引号变量
$Prefix = '[交付claw]'
...
Write-Host "${Prefix} Python: $pyver"

Write-Host "${Prefix} 安装/升级 pip 并安装 nanobot（可编辑模式）..."
...
$nodeVer = node -v
Write-Host "${Prefix} Node: $nodeVer"

Write-Host "${Prefix} 安装前端依赖（frontend）..."
...
Write-Host "${Prefix} 跳过根目录 npm install（dev 不再依赖 concurrently；避免公司网络/权限导致安装失败）"
...
if (-not (Test-Path $envLocal) -and (Test-Path $envExample)) {
    ...
    Write-Host "${Prefix} 已创建 frontend/.env.local（可按需修改 NEXT_PUBLIC_*）"
}

Write-Host ""
Write-Host "${Prefix} 安装完成。下一步在仓库根目录执行: npm run dev"
