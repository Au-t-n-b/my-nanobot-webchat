...
**交付物**：本手册 + `docs/sdui/schema-v2.json`（文档体）+ `docs/sdui/schema-v3.json`（含 `SduiPatch` / SSE 载荷）  
**协议对齐**：`docs/sdui/protocol.md` 第七章、`frontend/lib/sdui.ts`、`nanobot/web/skill_ui_patch.py`、`nanobot/agent/loop.py`（`emit_skill_ui_data_patch_event`）
...
- **手册（本文件）**：`docs/skill/dev-manual.md`
- **文档 JSON Schema（v2 体）**：`docs/sdui/schema-v2.json`
- **Patch / SSE 契约**：`docs/sdui/schema-v3.json`（`$defs/SduiPatch`、`$defs/SkillUiDataPatchSse`）
- **协议详解**：`docs/sdui/protocol.md` 第七章
...
- **静态文档结构**：仍以 `docs/sdui/schema-v2.json` 与 v2 手册节点速查为准。
- **Patch**：`docs/sdui/schema-v3.json` 中 **`SduiPatch`** 必填 **`docId`、`revision`、`ops`**；SSE 外层见 **`SkillUiDataPatchSse`**。
